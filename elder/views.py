...
def createReport(
    ...
    offset = pdf.x + 130
    pdf.multi_cell(
        90, 0, f'Phone Number (Patient) : {preProfile[0]["pphone"]}', 0, align="L"
    )

    pdf.y = top
    pdf.x = offset
    pdf.multi_cell(
        90, 0, f'Contact Number (Doctor) : {preProfile[0]["dphone"]}', 0, align="L"
    )
    ...
    pdf.ln(8)
    pdf.cell(
        epw, 0.0, f"Report Generated Duration : {a.date()} - {b.date()}", align="L"
    )
    pdf.ln(4)
    ...
    if countB != 0:
        pdf.cell(
            epw, 0.0, f"Beats per Minute Average : {int(bpmavg/countB)}", align="L"
        )
    else:
...
class ReportDataView(APIView):
    ...
    def post(self, request, format=None):
        ...
        x = datetime.now()
        preProfile = query(
            f"select * from elder_patientrelative where id='{request.data['patID']}'"
        )
        tempavg = 0
        bpmavg = 0
        spo2avg = 0
        ...
        if request.data["type"] == "dates" and request.data["dur"] == "total":
            a = datetime(
                request.data["fromY"], request.data["fromM"] + 1, request.data["fromD"]
            )
            b = datetime(
                request.data["toY"], request.data["toM"] + 1, request.data["toD"]
            )
            print(a, b)
            ...
            b = datetime(
                request.data["fromY"],
                request.data["fromM"] + 1,
                monthrange(request.data["fromY"], request.data["fromM"] + 1)[1],
            )
            ...
            b = datetime(
                request.data["fromY"], 12, monthrange(request.data["fromY"], 12)[1]
            )
            raw_query = f"""SELECT * FROM elder_vital
                        WHERE patid_id={request.data['patID']}
                        AND entered_at BETWEEN '{a}' AND '{b}'
    ...
    def post(self, request, format=None):
        ...
        if token:
            decoded_token = jwt.decode(
                token[4:], SIMPLE_JWT["SIGNING_KEY"], algorithms=SIMPLE_JWT["ALGORITHM"]
            )
            # print(decoded_token)
    ...
    def get(self, req, pk, format=None):
        ...
        if pk[0] == "W":
            ...
            if result == []:
                ...
                for i in result:
                    if (
                        i["entered_at"].year == x.year
                        and i["entered_at"].month == x.month
                    ):
                        finarr.append(i)
                if finarr == []:
            ...
            if result == []:
                ...
                for i in result:
                    if calendar.month_name[i["entered_at"].month] in finres:
                        ...
                        finres[calendar.month_name[i["entered_at"].month]] = [
                            i["tempval"],
                            i["spo2val"],
                            i["bpmval"],
                            i["bpval"],
...
class EmailView(APIView):
    def post(self, req):
        ...
        result = query(
            f"select email from elder_userprofile where id=(select patrel_id from elder_patientrelative where id='{patid}');"
        )[0]
        result2 = query(
...
class GetMessages(APIView):
    def get(self, req, pk, format=None):
        result = query(
            f"select * from elder_notification where patid_id='{pk}' and approved=False;"
        )
        if len(result) > 0:
            return Response(result)
...
class GetNotifiedPatient(APIView):
    def get(self, req, pk, format=None):
        ...
        if len(result) == 1:
            result1 = query(
                f"select patid_id,approved,rejected from elder_notification where staffid_id='{result[0]['id']}' and removefromlist=false;"
            )
            if len(result1) >= 1:
                for i in range(0, len(result1)):
                    appr.append(result1[i])

                    result2 = result2 + query(
                        f"select * from elder_patientrelative where id='{result1[i]['patid_id']}';"
                    )
...
class ImageViewset(APIView):
    ...
    def face_recog_setup_one(self, id):
        ...
        urllib.request.urlretrieve(
            f"https://patientmedia.s3.ap-south-1.amazonaws.com/{result[0]['imageFile']}",
            result[0]["imageFile"][6:],
        )
        ...
        try:
            ...
            self.known_face_encoding.append(
    ...
    def base64_file(self, req, name="dumb"):
        ...
        return ContentFile(
            ...
            name="{}.{}".format(name + str(req.data["patid"]), ext),
        )
